...
The availability of new type of data has induced consequent changes in various disciplines from social science (e.g. online social network analysis \comment{citation ?}) to geography (e.g. new insights into urban mobility or perspectives on ``smarter'' cities~\cite{batty2013big}) and economics (\comment{Anto un insight la dessus en eco ?}), without being always smooth at the interface of implied disciplines, as shows the example of the difficult understanding between physics and urban sciences (\cite{dupuy2015sciences})).

\comment{(Juste) trouver un moyen de décrire la collecte des données sans dire qu'on l'a fait effectivement : architecture uniquement ?}
