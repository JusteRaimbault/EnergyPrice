...
\comment{zip level data : check if stations without zip}

\comment{prices for different carburants and purchase mode $\rightarrow$ maybe take the most frequent for now ? on an example dataset (ts 1484114402), we have 89\% of credit (against cash on all records). For fuel type, it is more evenly distributed : 34\% for Regular, 22\% for Premium, 23\% for midgrade, 19\% for diesel. Maybe for the sake of simplicity work with Regular type and credit only. on the same example dataset, 96\% of stations have conjointly both, what gives a good coverage with this choice.}
