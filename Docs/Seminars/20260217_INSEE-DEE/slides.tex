...
\author[Raimbault and Bergeaud]{Juste Raimbault$^{1,2,3,4}$ and Antonin Bergeaud$^{5,6,7}$\smallskip\\
$^{1}$LaSTIG, Univ. Gustave Eiffel, IGN-ENSG\\
$^{2}$CASA, UCL\\
$^{3}$UAR 3611 CNRS ISC-PIF\\
$^{4}$UMR CNRS 8504 G{\'e}ographie-cit{\'e}s\\
$^{5}$HEC Paris\\
$^{6}$CEPR, LSE\\
$^{7}$Innovation Lab, Coll{\`e}ge de France\smallskip\\
...
Bergeaud, A., \& Raimbault, J. (2020). An empirical analysis of the spatial variability of fuel prices in the United States. \textit{Transportation Research Part A: Policy and Practice, 132}, 131-143.

\bigskip

\centering

\includegraphics[width=0.8\linewidth]{figures/citation.png}
...
\textit{Spatio-temporal variability of retail fuel prices can be linked to various aspects and issues: geographical properties of a particular energy market, characteristics of the transportation system, spatial inequalities, interactions between transportation and territories, \ldots}
...
$\rightarrow$ Focus on \textbf{geographical patterns and structures}

\medskip

$\rightarrow$ \textbf{Complementarity} of spatial analysis methods

\medskip

$\rightarrow$ \textbf{Interdisciplinary} viewpoint on a multi-dimensionnal system
...
\begin{itemize}
	\item we collect novel micro-data of retail fuel prices with full geographical coverage in the US
	\item we show the existence of different spatial regimes, superposed to a strong determination by state-level taxes and local socio-economic characteristics
	\item we introduce a minimal theoretical model explaining the effect of population density on prices
\end{itemize}
...
\textit{Crowdsourced Big Data as a ``new'' way to understand better complex socio-technical systems}

\bigskip


$\rightarrow$ Construction of a large scale dataset covering most of US fuel stations on two months, by collecting crowdsourced data available on online sources
...
\begin{itemize}
\item $41\cdot 10^6$ unique observations, between January and March 2017\\$\rightarrow$ 5,204,398 gas station - day observations for main purchase mode and regular fuel, used in the analysis
...
\item Regressing the log of price on a state fixed-effect explains 76\% of the variance
...
\medskip
\item It decreases with the vote for a Republican candidate: suggests a circular link
\medskip
\item Local socio-economic features have explanatory power even when removing State fixed effect
\end{itemize}
...
\begin{itemize}
	...
	\item Minimise computationally $C\left[h,J,\eta,P\right] = \sum_j f_j(\vec{p},h,J,\eta,P)^2$ with a genetic algorithm in R \cite{Scrucca2013ga}
\end{itemize}

}


\sframe{Computational solutions to the minimal model}{

With parameters $\eta = 1$, $P=0.8$, $J=200$ gas stations, prices for 3 population densities: (i) uniform; (ii) linear; (iii) shifted exponential.
...
$\rightarrow$ consistent with the empirical fact of \textbf{decreasing prices with population density}
...
$\rightarrow$ \textbf{Complementarity} of spatial analysis (Moran and GWR) and econometrics methods (Multi-level regression), and a \textbf{minimal theoretical model}

\medskip

$\rightarrow$ Possible design of \textbf{territory-targeted car-regulation policies}, allowing both sustainability and territorial equity
...
$\rightarrow$ Microscopic data analysis, calibration of consumer/producer behavior models
...
$\rightarrow$ Parametrisation of a large-scale ABM of the spatialized fuel market: investigation of  adaptive policies effects at the local and global level
...
$\rightarrow$ Insight into \textbf{spatio-temporal dynamics} of fuel price and their determinants with \textbf{novel ``big'' micro data}

\medskip

$\rightarrow$ \textbf{Complementary of approaches} and conclusions through GWR and multi-level modeling

\medskip

$\rightarrow$ Fundamental role of \textbf{spatio-temporal non-stationarity} and existence of typical stationarity scales: back on the importance of \textbf{fieldwork}
...
First order condition for the profit maximisation problem:
